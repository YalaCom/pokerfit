...
function installTiles(){
  ...
  for(const [id,title,desc,badge] of defs){
    let b=grid.querySelector(`[data-casino-game="${id}"]`);
    if(!b){
      b=document.createElement("button");b.dataset.casinoGame=id;b.dataset.category="slots";b.className=`v4-slot-tile v4-${id}`;
...
function bindTile(tile,id){
  if(!tile||tile.dataset.v4Bound)return;tile.dataset.v4Bound="1";
  tile.addEventListener("click",event=>{
    event.preventDefault();event.stopPropagation();
    try{showInfo(id);}catch(error){console.error("V4_SLOT_INFO",id,error);toast("Не удалось открыть слот. Попробуй ещё раз.");}
  });
}
...
function showInfo(id){
  ...
  const play=$("v4InfoPlay");if(play)play.onclick=()=>{try{renderSlot(id);}catch(error){console.error("V4_SLOT_PLAY",id,error);toast("Ошибка запуска слота");}};
}

function renderSlot(id){
  const c=catalog[id],body=$("casinoGameBody"),title=$("casinoGameTitle");if(!c||!body)return;if(title)title.textContent=`${c.icon} ${c.title}`;
  ...
  document.querySelectorAll("[data-cbet]").forEach(b=>b.onclick=()=>{const input=$("casinoBet");if(input)input.value=b.dataset.cbet;});const spinButton=$("v4Spin");if(spinButton)spinButton.onclick=()=>spin(id);if(id==="grandjackpot")refreshGameJackpot();enhance();
}
...
async function spin(id){
  if(busy)return;let amount;try{amount=readBet();}catch(e){return toast(e.message)}busy=true;const btn=$("v4Spin");if(!btn){busy=false;return;}btn.disabled=true;clearVisuals();slotFx("spin");haptic("medium");
  try{
  ...
  }catch(e){console.error("V4_SLOT_SPIN",id,e);toast(e.message)}finally{busy=false;btn.disabled=false;}
}
