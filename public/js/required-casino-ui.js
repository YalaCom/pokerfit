...
function ensureHomeWallet(){
  ...
  if(!wallet){
    ...
    wallet.innerHTML=`
      <button id="homeTopup"><small>ПОПОЛНИТЬ</small><b>500K</b><span>ЗА 1 ₽</span></button>
      <button id="homeWithdraw"><small>ВЫВЕСТИ</small><b>1M</b><span>ЗА 1 ₽</span></button>`;
    const hero=home.querySelector(".hero-card");hero?.insertAdjacentElement("afterend",wallet);
    $("homeTopup").onclick=()=>exchange("topup");
    $("homeWithdraw").onclick=()=>exchange("withdraw");
  }
  ...
  if(!jackpot){
    ...
    jackpot.innerHTML=`
      <div class="rhj-top"><span>💰 GRAND JACKPOT</span><em>1 / 250 000</em></div>
      <strong id="requiredHomeJackpotValue">0</strong>
      <small>Банк пополняется чистыми проигрышами игроков</small>
      <div class="rhj-bottom"><span>GRAND FORTUNE</span><b>ИГРАТЬ →</b></div>`;
    wallet?.insertAdjacentElement("afterend",jackpot);
    jackpot.onclick=()=>{nav("casino");setTimeout(showJackpotInfo,120);};
  }
}
...
function ensureJackpotTile(){
  ...
  if($("requiredJackpotTile"))return;
  const tile=document.createElement("button");tile.id="requiredJackpotTile";tile.className="required-jackpot-tile";tile.dataset.category="slots";
  ...
  grid.prepend(tile);tile.onclick=showJackpotInfo;
}

async function exchange(kind){
  const button=$(kind==="topup"?"homeTopup":"homeWithdraw");if(button)button.disabled=true;
  try{
    ...
    await refreshBootstrap();haptic("success");
...
function showJackpotInfo(){
  ...
  body.innerHTML=`
    <article class="required-jp-info">
      <div class="required-jp-art"><span>💰</span><i>GRAND</i></div>
      <small>FIT CASINO • PROGRESSIVE JACKPOT</small>
      <h3>GRAND FORTUNE</h3>
      <strong id="requiredInfoJackpot">0</strong>
      <p>Отдельный Jackpot-slot. Весь накопленный банк может выпасть только здесь. После выигрыша банк становится 0.</p>
      <div class="required-jp-rules">
        <div><small>ПОЛЕ</small><b>3×5</b></div><div><small>ЛИНИИ</small><b>20</b></div><div><small>MAX WIN</small><b>×1000</b></div><div><small>JACKPOT</small><b>1:250K</b></div>
      </div>
      <div class="required-jp-symbols">🍒 🔔 💎 👑 7️⃣ 🃏 🌟 💰</div>
      <div class="required-jp-bonus"><small>FORTUNE VAULT</small><b>3+ 🌟 запускают бонус. 🃏 WILD остаются на поле; символы 💰 усиливают бонус и могут добавить вращения.</b></div>
      <button id="requiredJpPlay" class="casino-main-btn">ИГРАТЬ 💰</button>
    </article>`;
...
function renderJackpotSlot(){
  ...
  body.innerHTML=`
    <div class="required-jp-machine">
      <div class="required-jp-meter"><small>GRAND JACKPOT</small><strong id="requiredGameJackpot">0</strong></div>
      <div class="required-jp-screen"><div id="requiredJpGrid" class="required-jp-grid">${cells}</div><div id="requiredBonusBadge" class="required-bonus-badge hidden">FORTUNE VAULT</div></div>
      <div id="requiredJpResult" class="casino-result">20 линий • WILD • SCATTER • BONUS</div>
    </div>
    <div class="required-jp-dock">
      <div class="casino-bet"><label>СТАВКА</label><input id="requiredJpBet" type="number" min="1000" max="5000000" step="1000" value="10000" inputmode="numeric"><div class="casino-presets"><button data-rjp-bet="10000">10K</button><button data-rjp-bet="50000">50K</button><button data-rjp-bet="100000">100K</button><button data-rjp-bet="500000">500K</button></div><div class="required-maxwin">MAX WIN <b id="requiredMaxWin">10M</b> + JACKPOT</div></div>
      <button id="requiredJpSpin" class="casino-main-btn">SPIN 💰</button>
    </div>`;
...
async function spinJackpot(){
  if(busy)return;const input=$("requiredJpBet");const amount=Math.floor(Number(input?.value||0));
  ...
  try{
    const d=await api("/api/casino/jackpot/spin",{bet:amount,requestId:crypto.randomUUID()});await wait(620);
    timers.forEach(clearInterval);renderGrid(d.result.grid);markBaseWins(d.result.base?.lines||[]);
...
async function playBonus(bonus){
  ...
  for(let i=0;i<(bonus.frames||[]).length;i++){
    const frame=bonus.frames[i];renderGrid(frame.grid,sticky);sticky=new Set((frame.sticky||[]).map(x=>x.join(":")));markSticky(sticky);
    if(badge)badge.textContent=`FORTUNE VAULT • ${i+1}/${bonus.frames.length} • ×${Number(frame.bonusMultiplier||1).toFixed(2)}`;
    setResult(`BONUS +${chipsShort(frame.payout||0)}`,Number(frame.payout||0)>0);await wait(260);
  }
  badge?.classList.add("hidden");
}

function renderGrid(grid,locked=new Set()){
  const cells=[...document.querySelectorAll(".required-jp-cell")];for(let r=0;r<3;r++)for(let c=0;c<5;c++){const i=r*5+c;if(cells[i]){cells[i].textContent=grid?.[r]?.[c]||"?";cells[i].classList.toggle("sticky",locked.has(`${r}:${c}`));}}
}
...
async function refreshAll(){
  try{const d=await api("/api/casino/jackpot/status"),pool=Number(d.pool||0),text=pool.toLocaleString("ru-RU");
    ["requiredHomeJackpotValue","requiredCasinoJackpotValue","requiredInfoJackpot","requiredGameJackpot","homeJackpotValue"].forEach(id=>{if($(id))$(id).textContent=text;});
