...
export class HoneyFruitsController{
  ...
  async #outro(total,bet){
    ...
    e.audio.play("bonusComplete",{volume:1});e.particles.emit("goldenExplosion",W/2,H*.45,{count:52,tint:0xffd14a});const tier=e.win.tier(total,bet);if(total>0)await e.win.present(total,bet,{tier});await this.#continueButton();
  }
