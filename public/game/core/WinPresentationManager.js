...
export class WinPresentationManager{
  ...
  async present(win,bet,{tier=null}={}){
    win=Math.max(0,Math.floor(Number(win)||0));if(!win)return;tier=tier||this.tier(win,bet);const compact=tier==="RETURN"||tier==="WIN",W=this.app.screen.width,H=this.app.screen.height;
    this.overlay.visible=true;this.overlay.alpha=0;this.overlay.position.set(W/2,H/2);this.overlay.title.text=tier;this.overlay.amount.text="0";this.overlay.burst.alpha=0;this.overlay.burst.scale.set(compact?.32:.18);this.overlay.dim.alpha=compact?.22:.74;
    ...
    if(compact)this.particles.emit("sparkBurst",W/2,H*.48,{count:10,tint:tier==="RETURN"?0x8db6d9:0xffd36a});
...
function duration(t){return t==="MAX WIN"?4.5:t==="EPIC WIN"?3.8:t==="MEGA WIN"?3.1:t==="BIG WIN"?2.4:t==="NICE WIN"?1.6:t==="WIN"?1.0:.72;}
