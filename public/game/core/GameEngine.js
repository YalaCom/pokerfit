...
export class GameEngine extends EventTarget{
  constructor({PIXI=window.PIXI,container,tg=window.Telegram?.WebApp,quality="AUTO"}={}){
    ...
    this.app=null;this.resources=null;this.config=null;this.reels=null;this.particles=null;this.win=null;this.layers={};this.gameId=null;this._resizeObserver=null;this._idleStarted=false;this.#wireState();
  }
  async init(){
    ...
    this._resizeObserver=new ResizeObserver(()=>{if(this.reels&&this.fsm.is(GameState.IDLE)){this.#buildScene(this._bonusScene===true);this.reels.layout(this.app.screen.width,this.app.screen.height);}});this._resizeObserver.observe(this.container);
  }
  async loadGame(gameId,onProgress=()=>{}){
    await this.init();
    if(this.fsm.current!==GameState.LOADING){if(!this.fsm.is(GameState.IDLE,GameState.ERROR))throw new Error(`GAME_LOAD_BLOCKED:${this.fsm.current}`);if(this.fsm.is(GameState.ERROR))this.fsm.transition(GameState.IDLE);this.fsm.transition(GameState.LOADING);}
    this.reels?.destroy();this.reels=null;this.particles?.destroy();this.particles=null;this.win=null;this._bonusScene=false;
    for(const layer of Object.values(this.layers))layer.removeChildren();
    try{
      this.gameId=gameId;this.resources=await this.assets.loadGame(gameId,onProgress);this.config=this.resources.config;this.#buildScene(false);
      ...
      this.audio.startBaseMusic();this.fsm.transition(GameState.IDLE);this.#idleLoop();return this.config;
    }catch(error){this.#toError();throw error;}
  }
  canSpin(){return this.fsm.is(GameState.IDLE,GameState.BETTING);}
  async presentSpin(serverResponse,{onBalance=()=>{},onStatus=()=>{}}={}){
    if(!this.canSpin())throw new Error(`SPIN_BLOCKED:${this.fsm.current}`);
    const result=serverResponse?.result,bet=Number(serverResponse?.bet||0);if(!result?.initialGrid||!bet)throw new Error("BAD_GAME_RESULT");
    try{
  ...
  async #playFreeSpins(bonus,bet,onStatus){
    this.fsm.transition(GameState.BONUS_TRIGGER);this.audio.play("bonus");this.haptics.bonus();this.fsm.transition(GameState.BONUS_INTRO);await this.#bonusIntro();await this.audio.enterBonusMusic();this._bonusScene=true;this.#buildScene(true);this.fsm.transition(GameState.FREE_SPINS);
    let i=0;for(const frame of bonus.frames||[]){
      i++;onStatus(`FREE SPIN ${i}/${bonus.frames.length} • MULTI x${frame.startMultiplier||2}`);this.reels.startSpin();
      ...
      for(const cascade of frame.cascades||[]){onStatus(`FREE SPIN ${i}/${bonus.frames.length} • TUMBLE x${cascade.multiplier}`);await this.reels.animateWins(cascade.wins||[]);await this.reels.animateCascade(cascade,this.particles);this.audio.play("burst");}await wait(150);
    }
    const total=Number(bonus.payout||0);if(total>0){const tier=this.win.tier(total,bet);await this.win.present(total,bet,{tier});}
    this.fsm.transition(GameState.BONUS_OUTRO);await this.#bonusOutro();await this.audio.exitBonusMusic();this._bonusScene=false;this.#buildScene(false);
  }
  async #bonusIntro(){
    const W=this.app.screen.width,H=this.app.screen.height,flash=new this.PIXI.Graphics().rect(0,0,W,H).fill({color:0xffd56a,alpha:0});this.layers.ui.addChild(flash);
    const title=new this.PIXI.Text({text:"GOLDEN ASCENSION",style:{fontFamily:"Arial",fontSize:Math.max(28,Math.min(58,W*.078)),fontWeight:"900",fill:0xffe69c,stroke:{color:0x3d1b00,width:6},letterSpacing:2,align:"center"}});title.anchor.set(.5);title.x=W/2;title.y=H/2;title.alpha=0;this.layers.ui.addChild(title);this.particles.emit("starBurst",W/2,H/2,{count:48});
    await timelineDone(gsap.timeline().to(flash,{alpha:.82,duration:.12}).to(flash,{alpha:0,duration:.35}).fromTo(title.scale,{x:.42,y:.42},{x:1,y:1,duration:.45,ease:"back.out(2.8)"},0).to(title,{alpha:1,duration:.18},0).to(title,{alpha:0,duration:.3},.92));flash.destroy();title.destroy();
  }
  ...
  #idleLoop(){if(this._idleStarted)return;this._idleStarted=true;let last=0;const tick=t=>{if(t-last>2800&&this.fsm?.is(GameState.IDLE)){last=t;this.particles?.emit("magicTrail",this.app.screen.width*(.15+Math.random()*.7),this.app.screen.height*(.18+Math.random()*.45),{count:this.quality==="LOW"?2:4,tint:0xd5b65f,speed:26,life:1.25});}requestAnimationFrame(tick);};requestAnimationFrame(tick);}
...
function format(n){return Math.floor(Number(n)||0).toLocaleString("ru-RU");}
