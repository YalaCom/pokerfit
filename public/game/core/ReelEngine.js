...
export class ReelEngine{
  ...
  destroy(){this.app.ticker.remove(this._ticker);for(const r of this.reels)try{r.blur?.destroy?.();}catch{}this.reels=[];this.layer.removeChildren();this.boardRoot=null;this.boardMask=null;this.dropLayer=null;}
  startSpin(){for(const reel of this.reels){gsap.killTweensOf(reel);gsap.killTweensOf(reel.container);reel.state=ReelState.ACCELERATING;reel.stateTime=0;reel.velocity=0;reel.container.y=0;reel.container.alpha=1;this.#setBlur(reel,true);}}
  async stopOnGrid(grid,{anticipationReel=-1,stopDuration=.48,anticipationDuration=.88,staggerMs=64,anticipationStaggerMs=145,onAnticipation=async()=>{},onReelStop=()=>{},onScatterStop=()=>{}}={}){
    if(!validGrid(grid,this.config.rows,this.config.reels))throw new Error("INVALID_REEL_GRID");await wait(430);
    for(let c=0;c<this.reels.length;c++){
      ...
      await this.#stopReel(reel,grid,c,anticipating?anticipationDuration:stopDuration);onReelStop(c);
      ...
      await wait(anticipating?anticipationStaggerMs:staggerMs);
    }
    this.setGrid(grid);
  }
  ...
  async animateCascade(cascade,particleManager){
    const removed=cascade.removed||[];for(const p of removed){const s=this.#spriteAt(p.r,p.c);if(!s)continue;const gp=s.getGlobalPosition();particleManager?.emit("symbolExplosion",gp.x,gp.y,{tint:0xffd775,count:this.quality==="LOW"?7:14});gsap.to(s,{alpha:0,duration:.15,ease:"power2.out"});gsap.to(s.scale,{x:s.scale.x*1.22,y:s.scale.y*1.22,duration:.15,ease:"power2.out"});}
    await wait(175);for(const reel of this.reels)for(let r=0;r<this.config.rows;r++){const s=this.#spriteAt(r,reel.index);if(s)s.alpha=0;}const temp=[];
    for(const d of cascade.drops||[]){const tex=this.textures[d.symbol];if(!tex)continue;const s=new this.PIXI.Sprite(tex);s.anchor.set(.5);this.#fitSprite(s,.8);s.x=this.#cellX(d.to.c);s.y=(d.from?this.#cellY(d.from.r):-(this.cellH*(.6+Math.random()*2.5)));s.alpha=d.new?.94:1;this.dropLayer.addChild(s);temp.push(s);gsap.to(s,{y:this.#cellY(d.to.r),duration:(d.new?.38:.28)+Math.random()*.1,ease:d.new?"back.out(1.6)":"power2.inOut"});}
    await wait(455);for(const s of temp)s.destroy();this.setGrid(cascade.nextGrid);for(const reel of this.reels)for(let r=0;r<this.config.rows;r++){const s=this.#spriteAt(r,reel.index);if(s)s.alpha=1;}await wait(70);
  }
  ...
  #build(){
    this.layer.removeChildren();this.reels=[];const cols=this.config.reels,rows=this.config.rows,dense=cols>=12;const horizontalPad=dense?Math.max(5,this.width*.012):Math.max(12,this.width*.035);this.gap=dense?Math.max(1,Math.min(2,this.width*.0025)):Math.max(2,Math.min(5,this.width*.006));
...
function mod(n,m){return ((n%m)+m)%m;}function shuffle(a){for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]];}return a;}
...
function validGrid(grid,rows,cols){return Array.isArray(grid)&&grid.length===rows&&grid.every(row=>Array.isArray(row)&&row.length===cols);}function wait(ms){return new Promise(r=>setTimeout(r,ms));}export {ReelState};
