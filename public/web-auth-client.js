...
function mountAuth(resolve,onStatus){
  ...
  root.querySelector("#fitTelegramLogin").onclick=async()=>{
    const button=root.querySelector("#fitTelegramLogin");button.disabled=true;setStatus(status,"Создаю одноразовый вход…");
    try{
      const link=await post("/telegram/start",{});setStatus(status,"Открой бота и подтверди вход. Эта страница ждёт подтверждение автоматически.");
      ...
      const check=async()=>{
        if(finished)return;
        if(Date.now()-started>5*60*1000){finished=true;button.disabled=false;setStatus(status,"Время входа истекло. Нажми кнопку ещё раз.",true);return;}
        try{
          const s=await post("/telegram/status",{token:link.token});
          if(s.authenticated){finished=true;finish();return;}
          if(s.status==="AWAITING_CONFIRM")setStatus(status,"Аккаунт найден. Нажми «Разрешить вход» в сообщении бота.");
...
function errorText(code){return ({BAD_WEB_LOGIN:"Логин: 3–32 символа, латиница/цифры/._-",BAD_DISPLAY_NAME:"Имя должно быть от 2 до 40 символов",BAD_WEB_PASSWORD:"Пароль должен быть от 8 до 72 символов",WEB_LOGIN_TAKEN:"Этот логин уже занят",WEB_LOGIN_FAILED:"Неверный логин или пароль",PLAYER_BANNED:"Аккаунт заблокирован",BAD_LOGIN_TOKEN:"Ссылка входа повреждена"})[code]||String(code||"Ошибка входа").replaceAll("_"," ");}
