...
test("Honey Fruits returns a server-authoritative 10x8 field",()=>{
  for(let i=0;i<12;i++){
    const r=createHoneyFruitsResult(10_000);
    assert.equal(r.initialGrid.length,8);
    assert.ok(r.initialGrid.every(row=>row.length===10));
...
test("Giant Bee valid positions never leave the 10x8 board",()=>{
  for(const [w,h] of HONEY_FRUITS_CONFIG.beeSizes){
    const positions=getValidBeePositions(w,h);assert.ok(positions.length>0);
    for(const p of positions){assert.ok(p.x>=0&&p.y>=0);assert.ok(p.x+w<=10);assert.ok(p.y+h<=8);}
  }
});
...
test("A naturally triggered Honey Bee Bonus preserves growth rules",()=>{
  ...
  for(const frame of bonus.frames){
    const b=frame.beeBefore;assert.ok(b.widthCells>=2&&b.widthCells<=6);assert.ok(b.heightCells>=2&&b.heightCells<=5);assert.ok(b.position.x+b.widthCells<=10);assert.ok(b.position.y+b.heightCells<=8);assert.equal(frame.collectedBees,frame.scatterPositions.length);
    ...
    for(const g of frame.growths){assert.equal(g.toLevel,g.fromLevel+1);assert.equal(g.extraSpins,2);assert.deepEqual([g.widthCells,g.heightCells],HONEY_FRUITS_CONFIG.beeSizes[g.toLevel]);assert.ok(g.position.x+g.widthCells<=10);assert.ok(g.position.y+g.heightCells<=8);}
