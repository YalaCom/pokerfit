...
export async function playMoreSlot(env,userId,slotId,bet,requestId){
  ...
  const bonus=triggered?bonusRound(cfg,bet,grid):null,payout=Math.min(Math.floor(bet*5000),Math.max(0,base.payout+(bonus?.payout||0))),multiplier=Math.floor(payout/Math.max(1,bet)*100)/100;
  const result={slotId:cfg.id,name:cfg.name,rows:cfg.rows,cols:cfg.cols,lineCount:cfg.lineCount,grid,base,bonusTriggered:triggered,bonus,payout,multiplier};
